...
В приложение добавлен watchdog локального SOCKS. Через 20 секунд после запуска, а затем примерно раз в 45 секунд, он проверяет, отвечает ли `127.0.0.1:18080`. Если локальный порт два раза подряд перестал принимать соединения, приложение пишет событие в журнал и перезапускает `olcrtc`. Это помогает при сценарии, когда через долгое время умирает именно локальный SOCKS-процесс. Если проблема в том, что внешний VPN-клиент держит сломанный системный туннель после смены сети, watchdog покажет это в диагностике, но не сможет обойти маршрутизационную петлю без `NetworkExtension`.
