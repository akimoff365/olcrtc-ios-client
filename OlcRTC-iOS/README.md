...
В интерфейсе есть карточка текущего состояния, активная сеть, счетчик переподключений, импорт из буфера обмена, открытие `olcrtc://` ссылок и быстрое копирование SOCKS-параметров.
